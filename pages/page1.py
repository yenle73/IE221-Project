...
if submit_button:
    if user_title_1 in all_titles and user_title_2 in all_titles and user_title_3 in all_titles:
        ...
        st.markdown(results.style.set_table_styles([dict(selector='*', props=[('text-align', 'center')]), dict(selector='th', props=[('max-width', '800px')])]).to_html(),unsafe_allow_html=True)
    else:
